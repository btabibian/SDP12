...
\begin{center}
\leavevmode
\includegraphics[width=0.3\textwidth] {AgentArch.png}
\end{center}
...
Perception is the way in which an agent understands the environment. In this project, robots were able to percept the environment using the overhead camera. Using the images provided by the camera we were able to calculate locations and orientations(hereafter called pos) of robot, Figure \ref{fig:robot} and opponent, ball location on the pitch and the pitch itself.
...
\end{align}
One can observe that equation \ref{orientationEq} produces results from $-\pi/2$ - $\pi/2$. This method cannot compute the heading of the calculated orientation vector, to disambiguate this issue I used one of the methods developed earlier. This method uses central moments of the image and the circle fitted around the object to calculate the orientation\ref{fig:CircleMethod}. Even though the latter method alone was not robust against noise but it was effective in deciding the heading of the calculated orientation using the former method. 
\begin{figure}[htp]
\begin{center}
\leavevmode
\includegraphics[width=0.15\textwidth] {CenOrien.png}
\end{center}
\caption{Calculating orientation using center of mass of object and surrounding circle}
\label{fig:CircleMethod}
\end{figure} 
\subsection{Results and Lessons learned}
We used this method during later tests, milestones and matches. Successful implementation of the solution was one of the keys to our success in milestone 3(score 70) and first friendly match(becoming semi-finalist),Figure \ref{fig:visionres}.
...
A few lessons learned from this part of the project.
\begin{itemize}
...
I started with implementing Potential Fields\cite{paper:PF} algorithm. This algorithm is a well-known algorithm for robot path planning. It has several important properties which makes it suitable for our application.The main advantage of this method is that it is completely reactive, if fed with noisy input in one cycle it will be able to recover in next cycles. Therefore performance of the algorithm degrades as amount of noise increases without complete failure.
\subsection{Abstract Simulation}
I haven't had access to a complete simulation environment of the arena or the actual robot with reliable input data when started working on this algorithm. Therefore, I implemented a simple forward kinematic of a non-holonimic differential drive robot(basic kinematic model of our robot) in Matlab and used it to test my implementation. With some changes this basic simulation was later used for development of a complete simulation environment.
\subsection{Challenges}
During implementation of this method the team faced several challenges. We started testing this algorithm in milestone two but we failed. The initial intuition for most of the members was that the failure was due to complexity of the algorithm however, later we realized that the failure was due to unreliable input data from vision stack. Later, team decided to abandon this idea and work on a simpler ideas. I continued working on this algorithm and also implemented extended potential field\cite{paper:OKhatib}, Figure \ref{fig:Epfs}. Once, vision stack become robust enough to provide reliable input I started testing the algorithm again. In the second attempt we observed a very good performance of the algorithm and used it as part of the agent implementation for the rest of competitions.
\subsection{Kinematic Model and Implementation}
To successfully implement potential field algorithm we also need to find a way to apply calculated velocity vector on the robot. To do so, I used the model described earlier to decompose the velocity vector into linear and angular velocities. The calculation is based on following equations and figure \ref{fig:KinModel}:
\begin{figure}[htp]
\begin{center}
\leavevmode
...
\end{align}
Integrating this layer with higher levels made our team distinguished from others in terms of robot control.
\subsection{Results and Lessons Learned}
...
Users of this implementation can also utilize its capabilities in obstacle avoidance scenarios using the documentation provided with code-base.
...
\end{itemize}
Source code of this part of the code base with relevant documentation is available on-line.\footnote{https://sites.google.com/site/btabibian/projects/sdp}
...
\begin{figure}[!bp]
\begin{center}
\subfigure[]{\includegraphics[width=0.125\textwidth] {step1.jpg}}
\subfigure[]{\includegraphics[width=0.125\textwidth] {step2.jpg}}
\subfigure[]{\includegraphics[width=0.125\textwidth] {step3.jpg}}
\subfigure[]{\includegraphics[width=0.125\textwidth] {Final.jpg}}
...
\begin{center}
\leavevmode
\includegraphics[width=0.5\textwidth] {robotimage.jpg}
\label{fig:robot}
\end{center}
\caption{Robot on the pitch}
\end{figure}

\begin{figure}[htp]
\begin{center}
\leavevmode
\includegraphics[width=0.3\textwidth] {vision.jpg}
\end{center}
\caption{An example image demonstrating locations and orientation of objects on the pitch.}
\label{fig:visionres}
\end{figure}
...
\begin{center}
\leavevmode
\includegraphics[width=0.5\textwidth] {PF-goal.png}
\end{center}
\caption{Potential vectors around destination position}
\label{fig:PFvectors}
\end{figure}
...
\begin{center}
\leavevmode
\includegraphics[width=0.5\textwidth] {pfs-fixed.png}
\end{center}
\caption{Simulated robot path produced by Potential Field algorithm and Extended Potential Field in presence of and obstacle.}
\label{fig:Epfs}
\end{figure}
